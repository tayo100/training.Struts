...
public class BarChartServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			dataset.executeQuery("SELECT DISTINCT YEAR(startDate) as Year, COUNT(YEAR(startDate)) as Total FROM TrainingProgram WHERE startDate IS NOT NULL AND YEAR(startDate)>2010 GROUP BY YEAR(startDate) ORDER BY YEAR(startDate)");

			chart = ChartFactory.createBarChart3D("Training Programs/Year", "Year", "Total", dataset, PlotOrientation.VERTICAL, true, true, false);
			
			chart.setTitle (new TextTitle("Training Programs/Year", new Font ("blackbody", Font.ITALIC, 14))); 
			...
			if (chart != null) {
				...
				final File file1 = new File(getServletContext().getRealPath(".") + "/groupchart.png");
