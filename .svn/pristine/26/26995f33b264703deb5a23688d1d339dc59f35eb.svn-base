...
<s:form method="post" action="%{#targetEntity}-document-details!update">
	...
	<colgroup><col width="120" /><col /></colgroup>
		...
		<tr>
			<td />
			<td><s:submit value="Update" /> <s:submit value="Remove" action="%{#targetEntity}-reportdocument-details!remove" cssClass="button-delete" onclick="javascript: return confirm('Are you sure you want to remove this record?');" /></td>
		</tr>
	</table>
</s:form>