<%@ include file="/common/taglibs.jsp"%>

<html>
<head>
<title>CDO Group Training Application Confirmation</title>
</head>
<body>
<table style="width: 100%">
	<colgroup>
		<col />
		<col width="350" />
	</colgroup>
	<tbody>
	<tr>
	<td>
	<table class="inputform">
		<colgroup>
			<col width="20%" />
			<col />
		</colgroup>
		<tr>
			<td>Application for:</td>
			<td><s:property value="announcement.title" /> - <s:property value="announcement.type" /></td>
		</tr>

		<tr><td></td><td colspan="2"><div class="button-bar">Dear <s:property value="cdoBioData.title" /> <s:property value="cdoBioData.lastName" />, <s:property value="cdoBioData.firstName" />, <br/><br/>
		
		Your registration form has been submitted successfully.
		<br/> <br/>A copy of your registration form submitted herein has been sent to your mailbox. 
		<br/><br/>You will be contacted in due course if need be.<br/><br/>Thank you.<br/><br/>IITA Capacity Development Office<br/><br/></div></td></tr>
		<tr><td colspan="2"></td></tr>
		<tr><td colspan="2" align="left"><s:include value="/WEB-INF/jsp/include/further-info.jsp" /></td></tr>
	</table>
</td>
<td style="padding-left: 10px;">
				<s:include value="/WEB-INF/jsp/include/personal-data.jsp" />
			</td>
				</tbody>
</table>
</body>
</html>