...
<s:form method="post" action="%{#targetEntity}-document!update">
	...
	<colgroup><col width="120" /><col /></colgroup>
		...
		<tr>
			<td class="ar">Publication Year:</td>
			<td><s:textfield name="entityDocument.document.pubYear" value="%{document.pubYear}" /></td>
		</tr>
		<tr>
			<td class="ar">Authors:</td>
			<td><s:textfield name="entityDocument.document.authors" value="%{document.authors}" /></td>
		</tr>
		<tr>
			<td><label>Open for access:</label></td>
			<td><s:select name="entityDocument.document.allowPublic" value="%{document.allowPublic}"
			list="#{'true':'Allow public view', 'false':'Hide from public'}" /></td>
